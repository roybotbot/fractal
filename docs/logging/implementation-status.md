...
`session/logger.py` — `ExecutionLogger`. Two targets: `execution_log.jsonl` (structural events) and `content_log/` (prompt/response markdown files). `node_path` computation at node registration. Flush after every execution log write. All event methods covering session, node, step, gate, signal, and LLM call lifecycle.
...
1. `detector/checks.py` — AST-based checks are straightforward Python, provide immediate value for testing gate logic.
2. `session/logger.py` — no LLM calls, no complex dependencies. Build early so every subsequent module can log from the start.
3. `runner/gates_runner.py` — testable against hand-constructed nodes with the schema layer.
4. `runner/context.py` — testable without live LLM calls.
5. `runner/runner.py` — core execution loop. Test with a mock LLM client and mock logger.
6. `planner/superpowers_parser.py` — no LLM calls, pure markdown parsing. Test against real superpowers plan output samples.
7. `planner/classifier.py` and `planner/decomposer.py` — requires live LLM calls.
8. `planner/planner.py` — wire classifier + decomposer + parser into both entry points.
9. `detector/drift.py` and `detector/uncertainty.py` — requires LLM judge for some checks.
10. `notify/` — self-contained, build and test independently.
11. `session/state.py` and `session/log.py` — straightforward serialization, build last.
12. CLI — wire everything together. Both `--task` and `--plan` entry points.
