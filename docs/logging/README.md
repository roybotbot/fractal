...
- [Superpowers integration](./superpowers-integration.md) — how this system plugs into the superpowers workflow at the handoff point between `write-plan` and `execute-plan`
...
- [Logging](./logging.md) — execution log (structural events, JSONL), content log (prompts and responses per step), node path hierarchy
